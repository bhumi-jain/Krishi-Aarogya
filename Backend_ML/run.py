...
app = Flask(__name__)
CORS(app)
# Define the class names (make sure this matches your model output classes)
class_names = ['Healthy', 'Brown Spot', 'Sheath Blight']  # Update with your actual class names

# Load the model (ensure correct path and architecture)
model = models.resnet18(weights=models.ResNet18_Weights.IMAGENET1K_V1)
num_ftrs = model.fc.in_features
model.fc = torch.nn.Linear(num_ftrs, len(class_names))  # Adjust to match your class count
model.load_state_dict(torch.load(r'ML_model/crop_disease_model1.pth'), strict=False)
model.eval()

# Define image preprocessing (resize, normalize, etc.)
transform = transforms.Compose([
    transforms.Resize(256),
...
# Define the route for health prediction
